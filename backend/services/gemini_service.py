...
class GeminiImageGenerator:
    ...
    def _build_reference_part(self, reference_image: Optional[str]) -> Optional[Dict]:
        if not reference_image or not reference_image.startswith("data:image/"):
            return None
...
Make it suitable for a professional social media marketing campaign."""
    ...
    async def generate_image(
        ...
        """
        try:
            reference_part = self._build_reference_part(reference_image)
                ...
                response = await client.post(
                    ...
                    json={
                        'contents': [
                            {
                                'parts': [
                                    *([reference_part] if reference_part else []),
                                    {
                                        'text': prompt
                                    }
                                ]
